...
import { IDashboardPayload, TopSpentData, TopSpentRoot, TopSpentUserColumn } from '@interfaces';
...
export const TopSpentUserTableModule = ({ limit, dashboardPayload }: IProps) => {
  const dispatch = useDispatch();
  const [topSpentUsers, setTopSpentUsers] = useState<TopSpentData>({ limit, count: 0, data: [] });
  ...
  const getTopSpentUsers = async (limitInp: number, page: number) => {
    try {
      ...
      const res: TopSpentRoot = await dashboardAPI.getTopSpentUsers({ limit: limitInp, page });
      const newResponse: TopSpentUserColumn[] = res.data.result.map((item, index) => {
        return {
          key: item.member_id,
          no: index + 1,
          name: item.name,
